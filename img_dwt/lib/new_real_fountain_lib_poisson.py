...
def poisson_randChunkNums(num_chunks):
...
class Droplet:
    ...
    def poisson_chunkNums(self):
        random.seed(self.seed)
        np.random.seed(self.seed)
        return poisson_randChunkNums(self.num_chunks)
...
class Fountain(object):
    ...
    def droplet(self):
        ...
        if not self.all_at_once:
            self.chunk_selected = poisson_randChunkNums(self.num_chunks)
        else:
...
class Glass:
    ...
    def addDroplet(self, drop):
        ...
        entry = [drop.poisson_chunkNums(), drop.data] if not self.all_at_once else [drop.all_at_once_chunkNums(), drop.data] 
...
def test_LT_fountain():
    testFile = 'poisson_' + time.asctime().replace(' ', '_').replace(':', '_')
    ...
    if not os.path.exists(test_dir):
            os.mkdir(test_dir)
    suffix_list = ['100.txt', '150.txt', '200.txt', '250.txt', '300.txt', '350.txt', '400.txt', '450.txt', '500.txt', '1000.txt', '1500.txt', '2000.txt', '2500.txt', '3000.txt']
...
def test_LT_fountain_short():
    testFile = 'poisson_short_' + time.asctime().replace(' ', '_').replace(':', '_')
    test_dir = os.path.join(
                NEW_SIM_PATH,
                "LT",
                testFile
