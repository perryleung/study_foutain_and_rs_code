...
def mrsd_randChunkNums(num_chunks):
    size = mrsd_func(num_chunks).__next__()
...
class Droplet:
    ...
    def mrsd_chunkNums(self):
        random.seed(self.seed)
        np.random.seed(self.seed)
        return mrsd_randChunkNums(self.num_chunks)
...
class Fountain(object):
    ...
    def droplet(self):
        ...
        if not self.all_at_once:
            self.chunk_selected = mrsd_randChunkNums(self.num_chunks)
        else:
...
class Glass:
    ...
    def addDroplet(self, drop):
        ...
        entry = [drop.mrsd_chunkNums(), drop.data] if not self.all_at_once else [drop.all_at_once_chunkNums(), drop.data] 
...
def test_LT_fountain():
    testFile = 'MRSD_' + time.asctime().replace(' ', '_').replace(':', '_')
